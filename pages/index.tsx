...
export default function Home() {
    const initResults = (size = blocks.length) => {
        return Array.from({ length: size - 1 }, () => Array.from({ length: size }, () => -1));
    };
    ...
    const getPrevBlockRule = (currBlock: number, prevRow: number[]) => {
        return [-1, 0, 1].map((n) => prevRow.at(currBlock + n) ?? prevRow.at(0)).join("");
    };

    const learnValue = (currBlock: number, currRowIndex: number, currRow: number[]) => {
        const prevRule = getPrevBlockRule(currBlock, currRowIndex === 0 ? blocks : currRow);
    ...
    const handleStart = async () => {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            results[i].forEach((_, bidx) => {
                arr = [...arr, learnValue(bidx, i, newArr[i - 1])];
            });
            newArr[i] = arr;
            setResults([...newArr]);
        ...
        <>
            ...
            <main className="container mx-auto items-center py-16">
                <div className="border-[0.2rem] border-solid border-yellow-500 bg-transparent p-4 flex items-center space-x-4">
                    ...
                    <div className="my-2">
                        <label className="mr-2" htmlFor="speed">
                    ...
                    <div className="my-2">
                        <p>NOTE: You may toggle the blocks in the first row to change their state</p>
                    </div>
                </div>
                ...
                <div className="flex align-center justify-center my-4 space-x-4">
                    <button
                        type="button"
                    ...
                    <button className="bg-white text-black px-4 py-2" onClick={async (e) => await handleStart()}>
                        Start
                    </button>
                    <button className="bg-white text-black px-4 py-2" onClick={() => setResults(initResults())}>
                        Reset
                    </button>
                </div>
                ...
                <div className="flex align-center justify-center mt-4 w-full">
                    {blocks.map((item, idx) => {
                        return (
                            <div
                                key={idx}
                                className={`border-yellow-300 border-[1px] border-solid`}
                                style={{
                                    backgroundColor: item === 0 ? "white" : "black",
                                    width: (867 / blocks.length).toString() + "px",
                                    height: (867 / blocks.length).toString() + "px",
                                }}
                                onClick={(e) => {
                                    let temp = [...blocks];
                                    temp[idx] = item === 0 ? 1 : 0;
                ...
                <div className="flex flex-col align-center justify-center w-full">
                    {results.map((row, idx) => {
                        return (
                            <div key={idx} className="flex align-center justify-center w-full">
                                {row.map((block, bidx) => {
                                    return (
                                        <div
                                            key={bidx}
