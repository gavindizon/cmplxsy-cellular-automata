            ...
            <div className="flex justify-center flex-wrap">
                {[7, 6, 5, 4, 3, 2, 1, 0].map((num, idx) => {
                    ...
                    return binaryString.split("").map((val, index) => (
                        <div key={index}>
                            <div
                                className={`w-8 h-8 border-2 border-yellow-300 ${(index + 1) % 3 === 0 ? "mr-7" : ""}`}
                                style={{ backgroundColor: KEY_COLOR[val] }}
                            ></div>
                            <div
                                className={`w-8 h-8 border-yellow-300 ${(index + 1) % 2 === 0 && "border-2"}`}
                                style={{
                                    backgroundColor: (index + 1) % 2 === 0 ? KEY_COLOR[ruleStr.charAt(idx)] : "",
                                }}
                            ></div>
                        </div>
                    ));
